...
# Serialize access to GPU model — only one inference or switch at a time
...
async def status_endpoint():
    """Returns the current readiness state of the backend. Kept async for responsiveness."""
...
def switch_model_endpoint(data: dict):
...
def chat_endpoint(data: dict):
    ...
    try:
        ...
        num_samples = max(1, min(int(data.get("num_samples", 5)), 10))
